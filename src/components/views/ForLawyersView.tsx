...
interface ChatConversation {
  ...
  unreadCount: number;
  avatar: string;
  messages: { id: string; sender: 'lawyer' | 'client'; text: string; time: string }[];
}
...
export const ForLawyersView: React.FC<ForLawyersViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'matches' | 'appointments' | 'messages' | 'cases' | 'profile' | 'analytics' | 'reviews' | 'settings'
  >('dashboard');
  ...
  useEffect(() => {
    async function loadIncomingConnections() {
      ...
      if (conns && conns.length > 0) {
        const mapped: MatchRequest[] = conns.map((conn) => {
          const st =
            conn.status === 'accepted'
              ? 'accepted'
              : conn.status === 'declined' || conn.status === 'rejected'
              ? 'declined'
              : 'pending';
        ...
        // Auto populate conversations for accepted matches
        const acceptedMatches = mapped.filter((m) => m.status === 'accepted');
        if (acceptedMatches.length > 0) {
          setConversations((existing) => {
            const nextConvs = [...existing];
            acceptedMatches.forEach((m) => {
              const exists = nextConvs.some((c) => c.connectionId === m.id || c.clientName === m.clientName);
              if (!exists) {
                nextConvs.push({
                  id: m.id,
                  connectionId: m.id,
                  clientName: m.clientName,
                  lastMessage: `Match accepted. Direct case chat active.`,
                  time: m.timeAgo || 'Active',
                  unreadCount: 0,
                  avatar: '',
                  messages: [],
                });
    ...
    // 10s polling backup
    const pollInterval = setInterval(loadIncomingConnections, 10000);
    ...
    if (supabase && currentUser?.userId) {
      const currentLawyerId = currentUser.userId;
      channel = supabase
        .channel(`lawyer_conns_${currentLawyerId}`)
        .on(
          'postgres_changes',
          {
            event: '*',
            schema: 'public',
            table: 'lawyer_connections',
  ...
  const handleAcceptMatch = async (id: string) => {
    ...
    const citizenId = match.citizenId || 'guest_citizen_101';
    ...
    try {
      ...
      console.warn('Error updating connection status to accepted:', e);
    }
    ...
    // Create or find dedicated conversation for messaging this client
    let targetConvId = '';
    setConversations((existing) => {
      const found = existing.find((c) => c.clientName === match.clientName);
      if (found) {
        targetConvId = found.id;
        return existing;
      }
      const newConvId = match.id;
      ...
      return [
        ...existing,
        {
          ...
          lastMessage: `Match accepted. Direct chat ready.`,
          time: 'Just now',
          unreadCount: 0,
          avatar:
            match.photoUrl ||
  ...
  const handleDeclineMatch = async (id: string) => {
    ...
    if (match) {
      ...
      try {
        ...
        console.warn('Error updating connection status to rejected:', e);
      }
    }
    ...
    setConversations((prev) =>
      prev.map((conv) => {
        if (conv.id === activeChatId) {
          ...
          return { ...conv, lastMessage: text, time: 'Just now', messages: newMsgs };
        }
        return conv;
      })
    );
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A] font-sans flex flex-col relative">
      ...
      <main className="flex-1 p-4 md:p-8 max-w-7xl w-full mx-auto space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-2xl p-5 shadow-2xs space-y-2">
                ...
                <div className="flex items-baseline justify-between">
                  <span className="text-2xl font-extrabold text-[#0F172A]">{conversations.length}</span>
                          ...
                          {req.status === 'pending' ? (
                            <>
                              <button
                                ...
                                className="px-3 py-1.5 bg-[#0A1628] hover:bg-[#D97706] text-[#FFFFFF] text-xs font-bold rounded-lg transition-colors cursor-pointer"
                              >
                                Accept & Connect
                              </button>
                              <button
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {req.status === 'accepted' && (
                                <button
                                  onClick={() => {
                                    let conv = conversations.find((c) => c.clientName === req.clientName);
                                    if (!conv) {
                                      const newConvId = req.id;
                                      conv = {
                                        id: newConvId,
                                        connectionId: req.id,
                                        ...
                                        time: 'Just now',
                                        unreadCount: 0,
                                        avatar: req.photoUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&auto=format&fit=crop&q=80',
                                        messages: [],
                                      };
                                ...
                                >
                                  ...
                                  <span>Message Client (संदेश भेजें)</span>
                                </button>
                              )}
                            </div>
                          )}
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      {req.status === 'pending' ? (
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => handleAcceptMatch(req.id)}
                            className="px-4 py-2 bg-[#0A1628] hover:bg-[#D97706] text-[#FFFFFF] text-xs font-bold rounded-xl cursor-pointer shadow-xs"
                          >
                            Accept & Connect
                          </button>
                          <button
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {req.status === 'accepted' && (
                            ...
                            >
                              <MessageSquare className="w-3.5 h-3.5" />
                              <span>Message Client (संदेश)</span>
                            </button>
                          )}
                        </div>
                      )}
                ...
                {appointments.map((ap) => (
                  <div key={ap.id} className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-2xl p-5 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => alert(`Calling client ${ap.clientName} at ${ap.contact}...`)}
        ...
        {/* ------------------------------------------------------------- */}
        {/* TAB 4: MESSAGES VIEW (DIRECT ADVOCATE CHAT INBOX) */}
        {/* ------------------------------------------------------------- */}
        {activeTab === 'messages' && (
          <div className="space-y-4">
            <div>
              <h1 className="text-xl font-extrabold text-[#0F172A]">Direct Messages & Consultations (संदेश)</h1>
              <p className="text-xs text-[#64748B]">Client message inbox and document exchange.</p>
            </div>

            {conversations.length === 0 ? (
              <div className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-3xl p-12 text-center space-y-3 shadow-2xs">
                <MessageSquare className="w-10 h-10 text-[#94A3B8] mx-auto" />
                <h3 className="text-sm font-bold text-[#0F172A]">No Active Conversations Yet</h3>
                <p className="text-xs text-[#64748B] max-w-sm mx-auto">
                  When you accept client match requests or citizens contact your profile, direct chat channels will open here.
                </p>
              </div>
            ) : (
              <div className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-3xl shadow-2xs grid grid-cols-1 md:grid-cols-12 min-h-[500px]">
                {/* Left Conversations List (4 cols) */}
                <div className="md:col-span-4 border-r border-[#E2E8F0] p-4 space-y-3">
                  <p className="text-xs font-bold text-[#64748B]">Active Conversations</p>
                      ...
                      <div
                        ...
                        className={`p-3 rounded-2xl cursor-pointer transition-all border ${
                          activeChatId === conv.id
                            ? 'bg-[#0A1628] text-[#FFFFFF] border-[#0A1628]'
                            : 'bg-[#F8FAFC] hover:bg-[#E2E8F0] text-[#0F172A] border-[#E2E8F0]'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-extrabold">{conv.clientName}</span>
                          <span className={`text-[10px] ${activeChatId === conv.id ? 'text-[#94A3B8]' : 'text-[#64748B]'}`}>
                            {conv.time}
                          </span>
                        </div>
                        <p className={`text-[11px] truncate mt-1 ${activeChatId === conv.id ? 'text-[#CBD5E1]' : 'text-[#64748B]'}`}>
                          {conv.lastMessage}
                        </p>
                      </div>
                    ))}
                ...
                {(() => {
                  ...
                  return (
                    <div className="md:col-span-8 flex flex-col p-2 md:p-4 bg-[#F8FAFC] rounded-r-3xl min-h-[500px]">
                      <DirectMessagePanel
                        connectionId={activeConv.connectionId || activeConv.id}
                        currentUserId={currentUser?.userId || 'lawyer_101'}
                        currentUserType="lawyer"
                        currentUserName={advocateName}
                        otherPartyName={activeConv.clientName}
                        compact={false}
                      />
                    </div>
                  );
                ...
                {cases.map((c) => (
                  <div key={c.id} className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-2xl p-5 shadow-2xs space-y-3">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        onClick={() => alert(`Updating case details for ${c.caseNumber}`)}
                      ...
                      <button
                        onClick={() => alert(`Uploading court order document for ${c.caseNumber}`)}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-[#FFFFFF] border border-[#E2E8F0] rounded-2xl p-5 shadow-2xs space-y-1">
                ...
                <p className="text-2xl font-extrabold text-[#0F172A]">{conversations.length}</p>
