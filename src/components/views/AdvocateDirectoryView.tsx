...
export const AdvocateDirectoryView: React.FC<AdvocateDirectoryViewProps> = ({
  ...
  const handleSendRequest = async () => {
    ...
    try {
      await createLawyerConnection(citizenId, requestTarget.id, generateUUID());
      trackEvent('lawyer_connection_requested', {
        lawyer_id: requestTarget.id,
        user_id: citizenId,
      });
      setToast(`✅ Consultation request bhej diya gaya — ${displayName(requestTarget)}`);
    } catch (err) {
      console.warn('Error sending consultation request:', err);
      setToast('⚠️ Request bhejne me problem aayi. Dobara try karein.');
    } finally {
